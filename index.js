...
const demoGalaxyData = {
  messages: [
    "Happy Birthday Trần Thị Kim Xuyến ❤️",
    "Sinh nhật tuổi 20, Chúc Kim Xuyến luôn vui tươi và rạng ngời ",
    "25/6",
  ],
  icons: ["♥", "💖", "❤️", "❤️", "💕", "💕"],
  colors: "#ff6b9d",
...
//       audio.loop = true; // Lặp lại nhạc khi hết
//       audio.play()
//         .then(() => {
...
function createTextParticle() {
  ...
  function animateParticle(timestamp) {
    ...
    if (progress < 1 && !heartAnimationStarted) {
      ...
      // Xóa phần tử sau khi hoàn thành hoạt ảnh
      if (element.parentNode) {
        element.parentNode.removeChild(element);
      }
      activeParticles.delete(element); // Đảm bảo xóa khỏi tập hợp
    }
  }
...
function startParticleAnimation() {
  const intervalId = setInterval(() => {
    if (heartAnimationStarted) {
      clearInterval(intervalId); // Dừng tạo hạt khi hoạt ảnh trái tim bắt đầu
      return;
    }
...
//     audio.load(); // Bắt đầu tải nhạc
//   });

//   audio.play(); // Phát nhạc sau khi nhạc đã tải xong
// }
...
document.addEventListener("mousemove", (e) => {
  if (isDragging) {
    ...
    const sensitivity = isMobile ? 0.5 : 0.5;
